...
* Create and manage a global session token (in addition to the user token) in the authserver. This is the approach taken by http://openid.net[OpenId Connect], and it does provide some options for SL, at the cost of some extra machinery. Note that all of the logout specs are still in draft form. Here are some links to the specs: http://openid.net/specs/openid-connect-session-1_0.html[Session Management], 
