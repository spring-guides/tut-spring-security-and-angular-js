...
In https://spring.io/tools/sts[Spring Tool Suite] (a set of Eclipse plugins) you can also create and import a project using a wizard at `File-&gt;New-&gt;Spring Starter Project`. Then jump to the link:#add-a-home-page[next section].
...
* All the CSS classes (apart from "ng-cloak") are from https://getbootstrap.com/[Twitter Bootstrap]. They will make things look pretty once we get the right stylesheets set up.
