...
The source code for the complete project we are going to build is in https://github.com/spring-guides/tut-spring-security-and-angular-js/tree/main/basic[Github here], so you can just clone the project and work directly from there if you want. Then jump to the link:#add-a-home-page[next section].
...
So far we have an application with a greeting that is hard coded. That's useful for learning how things fit together, but really we expect content to come from a backend server, so let's create an HTTP endpoint that we can use to grab a greeting. In your https://github.com/spring-guides/tut-spring-security-and-angular-js/blob/main/basic/src/main/java/demo/UiApplication.java[application class] (in "src/main/java/demo"), add the `@RestController` annotation and define a new `@RequestMapping`:

.UiApplication.java
[source,java]
----
include::src/main/java/demo/UiApplication.java[lines=17..27,indent=0]
----
...
We use Angular's `inject()` function to get an https://angular.io/guide/http[`HttpClient`] service, and use it to GET our resource. Angular passes us the response and we pull out JSON and assign it to the greeting.

To enable the `HttpClient` service in our application, we need to provide it in the application bootstrap:
