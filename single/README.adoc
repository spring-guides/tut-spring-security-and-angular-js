...
* There is a logout function exposed as a property of the component, which we can use later to send a logout request to the backend. If successful it sets a flag in the `app` service, and sends the user back to the login screen.
