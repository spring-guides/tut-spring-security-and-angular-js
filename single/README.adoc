...
It's not much different than the original in fact, but we replaced the `<html/>` body content (everything except the scripts) with a single `<app/>`, and added a `<base/>` element to the `<head/>` (it will be needed later by the router components).
