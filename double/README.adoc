...
== Run all apps within Docker

Running Redis within Docker is fine. We add now also the other apps to a docker-compose File (use the https://github.com/spring-guides/tut-spring-security-and-angular-js/blob/master/double/docker-compose-all.yml[`docker-compose-all.yml`]). Each app will run in its own container. Exposing just the port 8080 to the host will add additional security.

The host names have to be changed to Docker container name. This can be done by setting these OS Environment variables. They will overwrite the settings in your `application.yml` file:

 SPRING_REDIS_HOST
 SERVER_ADDRESS
 ZUUL_ROUTES_UI_URL
 ZUUL_ROUTES_ADMIN_URL
 ZUUL_ROUTES_RESOURCE_URL

All links between the apps need to be defined. To speed up the build, it is recommended to share the Maven cache between all apps using Docker volumes:
...
Add a temporary directory `m2cache` for the container shared Maven cache (as defined in the docker-compose file), start all containers and open the gateway app in the browser http://<docker-machine-ip>:8080/ .
