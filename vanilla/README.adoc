...
NOTE: It is a little bit of a risk to `permitAll()` access like this: there might be a handler that inadvertently sends sensitive data because it is not aware that the request is pre-flight. Spring MVC mitigates the risk (at the time of writing at least) by sending a 403 if it detects a pre-flight request that has no declared handler.
