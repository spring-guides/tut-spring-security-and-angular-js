...
https://github.com/spring-guides/tut-spring-security-and-angular-js/tree/master/single[source code in Github].
...
Spring Security makes it easy to handle the login request. We just need to add some configuration to our https://github.com/spring-guides/tut-spring-security-and-angular-js/blob/master/single/src/main/java/demo/UiApplication.java[main application class] (e.g. as an inner class):
...
The last thing we need to remember is to make the JavaScript components provided by Angular available anonymously to the application. We can do that in the `HttpSecurity` configuration above, and since it is static content, it's better to simply ignore it (together with the favicon that the browser likes to see):
