...
* The `AppModule` is defined in "hello.js" but is initialized explicitly in the HTML because it makes "hello.js" testable.
...
Most modern tutorials on Angular tend to use a node.js toolchain to download and repackage all the libraries and their dependencies. Some demos for Angular and other front end technologies often just include the library assets directly from the internet. Instead of doing either of those we are going to use https://webjars.org[Webjars] and install the third party assets in our application classpath. 

There are quite a few webjars needed for even the most basic Angular application:
...
NOTE: Using webjars for angular is a pragmatic choice, with strong pedagogical influences. Most Spring application developers are not familiar with the node.js toolchains, or TypeScript, which is what all the official Angular documentation uses now. So webjars and plain old JavaScript (ES5) are an easy way to get you started using tools familiar to a Java developer, but they will probably be limiting in the longer term. You might want to look into using `npm` and the Angular CLI for anything larger than the most basic application: if you just put the outputs from those toolchains in "src/main/resources/static" or "target/classes/static" then it will all work.
