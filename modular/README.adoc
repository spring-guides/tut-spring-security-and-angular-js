...
code,javascript
angular
    .module('hello', [ 'ngRoute', 'home', 'login' ])
    .config(
...
[source,javascript]
----
angular.module('hello', [ 'ngRoute', 'home', 'login' ]).config(
...
Before writing the `auth.js` code we can anticipate the changes in the other modules. First in `login.js` you should make the "login" module depend on the new "auth" module, and inject the "auth" service into the controller (and of course `$rootScope` is no longer needed):

[source,javascript]
----
angular.module('login', ['auth']).controller(
        'login',
...
----
angular
  .module('hello', [ 'ngRoute', 'auth', 'home', 'login' ])
  .config(
    ...
  }).run(function(auth) {
...
You simply need to convert the flag `authenticated` to a function call `authenticated()`, so that the "login" controller can reach into the "auth" service and find the value of the flag, now that it is not in `$rootScope`.
