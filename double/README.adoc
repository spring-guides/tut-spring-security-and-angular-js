...
Running Redis within Docker is fine. We add now also the other apps to a docker-compose File (use the https://github.com/spring-guides/tut-spring-security-and-angular-js/blob/master/double/docker-compose-all.yml[`docker-compose-all.yml`]). Each app will run in its own container. Exposing just the port 8080 to the host will give us additional security.

All host names have to be changed to Docker container names. This can be done by setting these OS Environment variables. They will overwrite the settings in your `application.yml` files:
...
Add a temporary directory for a container-shared-Maven-cache (`m2cache` as defined in the docker-compose file), start all containers and open the gateway app in your browser http://<docker-machine-ip>:8080/ .
